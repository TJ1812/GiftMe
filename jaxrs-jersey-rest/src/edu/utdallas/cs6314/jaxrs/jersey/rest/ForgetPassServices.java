...
@Path("/forgetpassservices")
public class ForgetPassServices {
	@Path("/forgetpass")
	@POST
	...
	public Response isValidUser(MultivaluedMap<String, String> formParam) {
		...
		try {
			...
			String query = "select * from user where email='" + email + "' and answer='" + answer + "'";
			...
			if (count >= 1) {
				...
				json.put("message", "Failed to answer correctly");
